...
object MockExampleSpec extends ZIOSpecDefault {
  ...
  def spec: Spec[Any, IOException] = suite("suite with mocks")(
    test("expect no call") {
      ...
      assertZIO(maybeTest1)(isUnit) *> assertZIO(maybeTest2)(isUnit)
    },
    test("expect no call on skipped branch") {
      ...
      assertZIO(noCallToConsole)(equalTo(42L)) *> assertZIO(noCallToClock)(equalTo("foo"))
    },
