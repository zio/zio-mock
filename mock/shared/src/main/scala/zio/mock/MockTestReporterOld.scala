...
object MockTestReporterOld {
  ...
  def render(
    ...
    println("************************************** HEEEELLLO)OOOOO:w")

    def loop(
        reportedEvent: ExecutionEvent,
        depth: Int,
      ...
      reporterEvent match {
        case SectionStart(labelsReversed, _, ancestors) =>
          println(s"""|:::SectionStart: 
                      | ::> labelsReversed = $labelsReversed
                      | ::> ancestors      = $ancestors""".stripMargin)
        ...
        case Test(labelsReversed, results, annotations, _, _, _) =>
          ...
          println(s"::: Annontations = $annotations")
          Seq(
            ExecutionResult(
              ResultType.Test,
    ...
    loop(reporterEvent, 0, List.empty, List.empty)
  }
